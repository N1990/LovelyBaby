...
public class LoginActivity extends BaseActivity {
        ...
        mController.doOauthVerify(this, media, new SocializeListeners.UMAuthListener() {
            ...
            public void onComplete(Bundle bundle, SHARE_MEDIA share_media) {
                if (bundle != null && !TextUtils.isEmpty(bundle.getString("uid"))) {
                    ...
                    Log.e("LoginActivity", "bundle = " + bundle.toString());
            ...
            OpenIdModel.handleOpenIdRequest(openId, new OkHttpClientManager.ResultCallback<OpenIdModel>() {
                ...
                public void onResponse(OpenIdModel response) {
                    if (response != null && !TextUtils.isEmpty(response.getData().getLoginAccount())) {
                        handleBundleThreeLogin(response.getData().getLoginAccount());
                    } else {
                        BundlePhoneActivity.newIntent(LoginActivity.this, openId, uid, userName, platform, 150);
                    }
                }
            });
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 150 && resultCode == RESULT_OK) {
            //处理绑定的问题
            handleBundleThreeLogin(getIntent().getStringExtra("phone"));
            return;
        }
    ...
    private void handleLogin(String phone, String pwd) {
        ...
        showWaitDialog();
        UserAssistModel.loginRequest(this, phone, pwd, platform, new OkHttpClientManager.ResultCallback<UserAssistModel>() {
            @Override
    ...
    private void handleBundleThreeLogin(String phone) {
        showWaitDialog();
        UserAssistModel.loginBundleRequest(this, phone, openId, uid, userName, platform, new OkHttpClientManager.ResultCallback<UserAssistModel>() {
            @Override
            public void onError(Request request, Exception e, String response) {
                hideWaitDialog();
                if (TextUtils.isEmpty(response)) {
    ...
    public static void newIntent(Context context, String phone, String psw) {
        Intent intent = new Intent(context, LoginActivity.class);
