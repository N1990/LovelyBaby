...
public class UnpayOrderFragment extends BaseFragment{
    ...
    private void addListener(){
        ...
        adapter.setOnHandlerListener(onHandlerLisetner);
    ...
    private CustomListener.ItemClickListener onHandlerLisetner = new CustomListener.ItemClickListener() {
        @Override
        public void onItemClick(View v, int position, Object object) {
            OrderListModel.DataEntity.RowsEntity itemData = (OrderListModel.DataEntity.RowsEntity) object;
            listViewHandler(itemData, position);
        }
    };
     ...
     * @param data
     */
    public void listViewHandler(OrderListModel.DataEntity.RowsEntity data, int position) {
        ...
        switch (status) {
            case WEI_ZHI_FU:
                ...
                if(price == 0){
                    showCustomDialog(orderCode, position);

                }else{
                    Intent pay = new Intent(getActivity(), PayActivity.class);
                    pay.putExtra("order_title", data.getTitle());
                    pay.putExtra("order_price", data.getPrice());
                    pay.putExtra("orderCode", data.getOrderCode());
                    startActivityForResult(pay, HANDLER_ORDER_REQUEST);
                }
                break;
            case YI_ZHI_FU:
    ...
    private void showCustomDialog(final String orderCode, final int position){
        builder = CustomDialogBuilder.getInstance(getActivity()).withTitle("取消订单")
                ...
                .withComfirmText("确认", new CustomListener.DialogListener() {
                    ...
                    public void onClick(View v) {
                        if(builder != null)
                            builder.setDialogDismiss();
                        handleCancelRequest(orderCode, position);
                    }
                })
     ...
     * @param position
     */
    private void handleCancelRequest(String orderCode, final int position){
        showWaitsDialog();
        HashMap<String, String> params = new HashMap<>();
